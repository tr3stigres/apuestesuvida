...
function link_titulo($titulo){
	...
	$destino=array('_','a','e','i','o','u','a','e','i','o','u','n','c','_','_','o');
...
if(isset($_REQUEST["t"]) && trim($_REQUEST["t"])!=""){
	...
	while($i<$sizeof_csr_tags){
		$csr_menu=$ConnMySQL->db_selectData("select c.idcajon, c.titulo, date_format(c.fecha_creacion,'%d/%m/%Y %H:%i') as fecha, c.html from cajon_de_sastre c, cajon_tags t where c.idcajon = t.idcajon and t.idtag=" . $csr_tags[$i]["idtag"] . " order by c.fecha_creacion desc");
		$j=0;
...
$arr_code=explode("embed/",$csr_menu[$j]["html"]);
$arr_code=explode('"',$arr_code[1]);
$arr_salida[$j]["code"]=$arr_code[0];
			$j++;
		}
		$tags.=$csr_tags[$i]["tag"] . " ";
		$i++;
