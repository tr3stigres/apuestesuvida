<?
	...
	/**
	 * Clase que detecta el navegador y el sistema operativo
	 * 
	 * @author JGX
	 * @version 1.0
	 */
	/*
		...
		*******************************************/
		...
		// main script, uses two other functions, $this->get_os_data() and $this->get_item_version() as needed
		// Optional $test_excludes is either null or one of the above values
	class DetectarNavegacio {
		var $navegador='';
		var $versio='';
		var $sistema_operatiu='';
		var $versio_so='';
		var $navegador_win_ko=array(
								"explorer"=>array(10,11)
								);
		var $navegador_win_ok=array(
								"firefox"=>array(15),
								"explorer"=>array(8),
								"opera"=>array(18),
								"chrome"=>array(30),
		...
		var $navegador_mac_ok=array(
								"firefox"=>array(22),
								"opera"=>array(18),
								"chrome"=>array(30),
								"safari"=>array(534)
								);
		var $navegador_linux_ok=array(
								"firefox"=>array(22),
								"opera"=>array(18),
								"chromium"=>array(1)
								);
		var $sistema_windows_ok=array('2000', 'xp', 'server 2003', 'vista', '7','8');
		var $info_txt="";
		
		function var_dump_a_string($array, $nivel=0){
			$txt="";
			foreach ($array as $clau=>$valor){
				if(is_array($valor)){
					$nou=$nivel+1;
					$txt.=$clau . "[" . substr($this->var_dump_a_string($array[$clau], $nou),0,-7) . "]; ";
				}else{
					if($nivel>0){
						$txt.=$valor . ",&nbsp;";
					}else{
		...
		function DetectarNavegacio(){
			$info=$this->browser_detection( 'full_assoc' );
			$this->info_txt=$this->var_dump_a_string($info);
			

			switch ($info["browser_working"]){
				case "moz":
					$this->navegador="firefox";
					$ver=explode('.',$info["moz_data"][1]);
					$this->versio=$ver[0];
					break;
				case "ie":
					$this->navegador="explorer";
					$this->versio=$info["browser_number"] . "<br>";
					if (strstr($_SERVER['HTTP_USER_AGENT'], "Trident/4.0" )) $this->versio="8";
					if (strstr($_SERVER['HTTP_USER_AGENT'], "Trident/5.0" )) $this->versio="9";
					if (strstr($_SERVER['HTTP_USER_AGENT'], "Trident/7.0" )) $this->versio="11";
					break;
				case "op":
					$this->navegador="opera";
					$ver=explode('.',$info["browser_number"]);
					$this->versio=$ver[0];
					break;
				case "webkit":
					$this->navegador=$info["webkit_data"][0];
					$ver=explode('.',$info["webkit_data"][1]);
					$this->versio=$ver[0];
					break;
				default:
					$this->navegador='';
			...
			switch ($info["os"]){
				case "win":
					$this->sistema_operatiu='windows';
					$this->versio_so=$info["os_number"];
					break;
				case "nt":
					$this->sistema_operatiu='windows';
					if($info["os_number"]==5) $this->versio_so='2000';
					if($info["os_number"]==5.1) $this->versio_so='xp';
					if($info["os_number"]==5.2) $this->versio_so='server 2003';
					if($info["os_number"]==6.0) $this->versio_so='vista';
					if($info["os_number"]==6.1) $this->versio_so='7';
					if($info["os_number"]==6.2 || $info["os_number"]=='') $this->versio_so='8';  //JGX(24/03/2014) el '' trobat a partir d'una prova de Robert
		    		break;
				case "mac":
					$this->sistema_operatiu='mac';
					$this->versio_so=$info["os_number"];
					break;
				case "lin":
					$this->sistema_operatiu='linux';
					$this->versio_so='';
					break;
				default:
					$this->sistema_operatiu=$info["os"];
					$this->versio_so=$info["os_number"];
					
			}
			//echo $this->navegador . " " . $this->versio . " " . $this->sistema_operatiu . " " . $this->versio_so;
			
			/*
			["browser_working"]=> string(3) "moz" 
			["browser_number"]=> float(9) 
			["ie_version"]=> string(0) "" 
			["dom"]=> bool(true) 
			["safe"]=> bool(true) 
			["os"]=> string(2) "nt" 
			["os_number"]=> float(5.1) 
			["browser_name"]=> string(5) "gecko" 
			["ua_type"]=> string(3) "bro" 
			["browser_math_number"]=> string(1) "9" 
			["moz_data"]=> array(5) { 
				[0]=> string(7) "firefox" 
				[1]=> string(5) "9.0.1" 
				[2]=> float(9) 
			...
			["mobile_test"]=> string(0) "" 
			["mobile_data"]=> string(0) "" 
			["true_ie_number"]=> string(0) "" 
			["run_time"]=> string(10) "0.00031281" 
			["html_type"]=> int(3) 
			["engine_data"]=> array(3) { 
				[0]=> string(5) "gecko" 
				[1]=> string(5) "9.0.1" 
		...
		function validar_opcions(){
			
			$valor=false;
			if($this->sistema_operatiu=='windows'){
				if(isset($this->navegador_win_ok[$this->navegador][0]) && $this->versio>=$this->navegador_win_ok[$this->navegador][0]){
					if(!isset($this->navegador_win_ko[$this->navegador][0]) || $this->versio!=$this->navegador_win_ko[$this->navegador][0]){
						$i=0;
						while($i<sizeof($this->sistema_windows_ok) && $this->sistema_windows_ok[$i]!=$this->versio_so){
							$i++;
						}
						if(isset($this->sistema_windows_ok[$i]) && $this->sistema_windows_ok[$i]==$this->versio_so) $valor=true;
					}
				}
			}
			if(isset($this->navegador_linux_ok[$this->navegador][0]) && $this->sistema_operatiu=='linux' && $this->versio>=$this->navegador_linux_ok[$this->navegador][0]) $valor=true;
			if(isset($this->navegador_mac_ok[$this->navegador][0]) && $this->sistema_operatiu=='mac' && $this->versio>=$this->navegador_mac_ok[$this->navegador][0]) $valor=true;
			
			return $valor;
		}
		
		function validar_prixnet(){
			
			$valor=false;
			if(isset($this->sistema_operatiu) && $this->sistema_operatiu=='windows'){
				if(isset($this->navegador) && $this->navegador=="explorer") $valor=true;
			}
				...
				*/
				if ( !is_array( $os_working_data ) 
					...
					switch ( $os_working_type ) {
						...
						case 'nt':
							// windows 8
							if ( strstr( $pv_browser_string, 'nt 6.2' ) ) {
								$os_working_number = 6.2;
							}
							// windows 7
							elseif ( strstr( $pv_browser_string, 'nt 6.1' ) ) {
		...
		*/
		function check_is_mobile( $pv_browser_user_agent )
		{
			...
			*/
			...
			identical to the list of $this->get_mobile_data
			*/
			// os
			...
			// then do basic mobile type search, this uses data from: $this->get_mobile_data()
